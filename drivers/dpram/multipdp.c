...
/* added wake_lock time for RIL to change the waketime for implementing Fast Dormancy LCD On/Off Timer */
#define DEFAULT_RAW_WAKE_TIME	(6*HZ)
static long pdp_wake_time;	/* jiffies */ /* wake time for not fmt packet */
...
#endif
		unsigned char *prx_buf = kzalloc(len, GFP_KERNEL);
